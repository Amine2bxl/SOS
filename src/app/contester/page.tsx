...
import { ASSO, COMMUNES, MOTIFS, DEMANDES_STANDARD } from "@/lib/data";
...
/** Remplace les champs vides par un repère explicite à compléter à la main. */
const ou = (valeur: string, repere: string) => (valeur.trim() ? valeur.trim() : `[${repere}]`);

const formatDate = (iso: string) => {
  if (!iso) return "[date du constat]";
  const d = new Date(iso);
  return Number.isNaN(d.getTime()) ? iso : d.toLocaleDateString("fr-BE");
};

function construireLettre(f: Form): string {
  ...
  const lignes = [
    ...
    f.communeConstat ? `Commune du constat : ${f.communeConstat}` : "[Destinataire indiqué sur votre courrier]",
    "",
    `Le ${aujourdhui}`,
    "",
